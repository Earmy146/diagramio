...
function EditorCanvas() {
  ...
  const handleConnect = useCallback(
    (connection) => {
      ...
      const sourceCol = sourceNode?.data?.columns?.find(
        (c) =>
          c.id === connection.sourceHandle ||
          c.name === connection.sourceHandle,
      );
      const targetCol = targetNode?.data?.columns?.find(
        (c) =>
          c.id === connection.targetHandle ||
          c.name === connection.targetHandle,
      );

      if (sourceCol && targetCol) {
        if (sourceCol.dataType !== targetCol.dataType) {
          toast.error(
            `Lỗi Kiểu Dữ Liệu! ${sourceCol.dataType} không thể tham chiếu vào ${targetCol.dataType}`,
          );
          return;
        }
      } else {
  ...
  // Tính toán hiệu ứng Highlight khi lựa chọn node hoặc tìm kiếm
  const selectedNodeIds = useMemo(
    () => nodes.filter((n) => n.selected).map((n) => n.id),
    [nodes],
  );

  // Tìm các bảng matching tìm kiếm
  const searchMatchNodeIds = useMemo(() => {
    if (!searchTerm.trim()) return new Set();
    const lowerSearch = searchTerm.toLowerCase();
    return new Set(
      nodes
        .filter((n) => n.data?.name?.toLowerCase().includes(lowerSearch))
        .map((n) => n.id),
    );
  }, [nodes, searchTerm]);

  const { computedNodes, computedEdges } = useMemo(() => {
    // Nếu có tìm kiếm, ưu tiên highlight các bảng matching
    if (searchTerm.trim() && searchMatchNodeIds.size > 0) {
      return {
        computedNodes: nodes.map((n) => {
          if (searchMatchNodeIds.has(n.id)) {
            return {
              ...n,
              data: {
                ...n.data,
              ...
              className:
                "opacity-100 transition-all duration-300 drop-shadow-2xl z-50",
            };
          }
          return {
            ...n,
            ...
            className:
              "opacity-30 grayscale-[50%] transition-opacity duration-300",
          };
        }),
        computedEdges: edges.map((e) => {
          // Highlight edges nối các bảng matching
          const isConnected =
            searchMatchNodeIds.has(e.source) ||
            searchMatchNodeIds.has(e.target);
          return {
            ...e,
            animated: isConnected,
            style: {
              ...e.style,
              stroke: isConnected ? "#fbbf24" : "var(--glass-border)",
  ...
  }, [nodes, edges, selectedNodeIds, searchTerm, searchMatchNodeIds]);
      ...
      <div className="absolute top-[-56px] right-4 z-50 flex items-center gap-3">
        ...
        <div className="relative flex items-center">
          ...
          <input
            ...
            className="pl-9 pr-8 py-1.5 bg-surface hover:bg-white/5 border border-glass text-textMain rounded-md transition-all text-sm placeholder-textMuted focus:outline-none focus:border-primary-400 focus:ring-1 focus:ring-primary-400 w-48"
          />
          {searchTerm && (
            <button
              onClick={() => setSearchTerm("")}
              className="absolute right-2 p-0.5 hover:bg-white/10 rounded-sm transition-all"
            >
              <X className="w-3.5 h-3.5 text-textMuted hover:text-textMain" />
            </button>
          )}
        </div>
        ...
        {isDirty && (
          <span className="text-xs text-yellow-500 font-medium tracking-wide animate-pulse">
            CHƯA LƯU
          </span>
        )}
        <button
