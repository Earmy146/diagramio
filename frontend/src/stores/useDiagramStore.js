...
const useDiagramStore = create((set, get) => ({
  ...
  layoutMode: "zone", // zone, linear, topological
  ...
  setDiagramData: (payload) => {
    set({
      ...
      layoutMode: payload.layoutMode || "zone",
      isDirty: false,
    });
  },
  ...
  setLayoutMode: (layoutMode) => set({ layoutMode, isDirty: true }),
  ...
  updateTableNode: (id, dataUpdater) =>
    set((state) => ({
      ...
      const mapType = (typeRaw, from, to) => {
        ...
        if (from === "POSTGRESQL" && to === "MYSQL") {
          if (t.startsWith("TIMESTAMP"))
            return t.replace("TIMESTAMP", "DATETIME");
