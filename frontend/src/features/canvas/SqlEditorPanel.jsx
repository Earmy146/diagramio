...
export default function SqlEditorPanel() {
  ...
  const [code, setCode] = useState('-- Nhập hoặc dán script CREATE TABLE của bạn vào đây\n\n')
  const [loading, setLoading] = useState(false)

  // Diagram store access
  const { nodes, edges, setNodes, setEdges, dialect, changeDialect } = useDiagramStore()
  ...
  const handleOpenExport = async () => {
    ...
    try {
      ...
      toast.error(err.message || 'Lỗi xuất SQL')
  ...
  const handleRunImport = async () => {
    ...
    try {
      const res = await parseSqlToErdAPI(code, dialect)
      if (res.success) {
      ...
      toast.error(err.message || 'SQL Cú pháp sai hoặc không chứa CREATE TABLE')
    } finally {
      setLoading(false)
    }
  }
          ...
          >
            ...
            <div className="h-14 border-b border-glass px-4 flex items-center justify-between">
              <h3 className="font-semibold text-textMain flex items-center gap-2">
                {mode === 'import' ? <Code2 className="w-4 h-4 text-primary-500" /> : <Save className="w-4 h-4 text-accent-500" />}
                {mode === 'import' ? 'Nhập SQL Script' : 'Xuất mã SQL'}
              </h3>
              <button onClick={() => setIsOpen(false)} className="p-1 rounded-md text-textMuted hover:bg-white/10 hover:text-textMain">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="px-4 py-2 bg-black/10 flex items-center justify-between">
              ...
              >
                ...
                <option value="SQLITE" disabled={mode === 'export'}>SQLite</option>
              </select>
