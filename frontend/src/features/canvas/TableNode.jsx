...
const TYPE_SUGGESTIONS = {
  MYSQL: ['INT', 'BIGINT', 'VARCHAR(255)', 'TEXT', 'LONGTEXT', 'DATETIME', 'DATE', 'BOOLEAN', 'DECIMAL', 'FLOAT', 'JSON'],
  POSTGRESQL: ['INTEGER', 'BIGINT', 'VARCHAR(255)', 'TEXT', 'TIMESTAMP', 'DATE', 'BOOLEAN', 'NUMERIC', 'REAL', 'JSONB', 'UUID'],
...
const TableNode = ({ id, data, selected }) => {
  ...
  const handleDeleteColumn = (colId) => {
    updateTableNode(id, { columns: data.columns.filter(c => c.id !== colId) })
  }
  ...
  return (
    <div className={`w-64 bg-surface rounded-xl border-2 transition-shadow shadow-lg group ${
      selected ? 'border-primary-500 shadow-primary-500/30' : 'border-glass'
    }`}>
      {/* Cung cấp danh sách Datalist ngầm định cho input combobox */}
      <datalist id={listId}>
        {TYPE_SUGGESTIONS[dialect]?.map(t => <option key={t} value={t} />)}
      </datalist>

      {/* Target Handle ở trên (đón kết nối tới) */}
      <Handle type="target" position={Position.Top} className="!w-16 !h-2 !bg-primary-500 !rounded-full !border-none !-top-1 px-2" />
      ...
      >
        <div className="flex items-center w-full">
          ...
          />
          {data.isDraft && <span className="text-[10px] bg-yellow-500/20 text-yellow-500 px-1.5 py-0.5 rounded ml-2">DRAFT</span>}
        </div>
      ...
      <div className="flex flex-col p-1 bg-surface/50">
        {data.columns?.map((col, idx) => (
          <div key={col.id || idx} className="flex items-center justify-between px-2 py-1.5 hover:bg-white/5 rounded-md text-sm group relative">
            ...
            <div className="flex items-center gap-1.5 flex-1 min-w-0">
              <button 
                onClick={() => handleColumnUpdate(col.id, 'isPrimary', !col.isPrimary)}
              ...
              <input 
                ...
                className={`w-full bg-transparent border-none outline-none nodrag px-1 ${
                  col.isPrimary ? 'font-medium text-textMain' : 'text-textMuted font-medium'
                }`}
            ...
            <div className="flex items-center gap-1 w-[90px] flex-shrink-0 justify-end relative">
              <input 
                value={col.dataType || ''}
                list={listId}
                onChange={(e) => handleColumnUpdate(col.id, 'dataType', e.target.value.toUpperCase())}
      ...
      <Handle type="source" position={Position.Bottom} className="!w-3 !h-3 !bg-accent-500 !border-2 !border-surface !-bottom-1.5" id="table-handle" />
    </div>
  )
}
